...
function get_pkg(){
	...
	git clone https://github.com/dinde/opennms-plugin-weathermap.git --branch v1.1-stable --single-branch opennms-plugin-weathermap
	if [ "$?" != "0" ] ; then
		echo "WARNING: git is not installed ! Trying to wget it."
		wget https://github.com/dinde/opennms-plugin-weathermap/archive/v1.1-stable/master.zip -O opennms-plugin-weathermap.zip
		if [ "$?" != "0" ] ; then
